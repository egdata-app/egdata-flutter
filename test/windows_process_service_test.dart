...
void main() {
  group('WindowsProcessService', () {
    group('hasProcessInDirectory', () {
      ...
      test(
        'returns true when process exists in directory (Windows)',
        () {
          if (Platform.isWindows) {
      ...
      test(
        'returns false for non-existent directory (Windows)',
        () {
          if (Platform.isWindows) {
            final result = WindowsProcessService.hasProcessInDirectory(
      ...
      test(
        'handles path with forward slashes (Windows)',
        () {
          if (Platform.isWindows) {
            final dartExe = Platform.resolvedExecutable;
    ...
    group('isProcessRunning', () {
      ...
      test(
        'returns true for running process (Windows)',
        () {
          if (Platform.isWindows) {
      ...
      test(
        'returns false for non-existent process (Windows)',
        () {
          if (Platform.isWindows) {
      ...
      test(
        'is case-insensitive for process name (Windows)',
        () {
          if (Platform.isWindows) {
            final result = WindowsProcessService.isProcessRunning('DART.EXE');
            expect(result, isTrue);
    ...
    group('getAllProcessPaths', () {
      ...
      test(
        'returns non-empty list on Windows',
        () {
          if (Platform.isWindows) {
            final result = WindowsProcessService.getAllProcessPaths();
      ...
      test(
        'all paths are valid file paths (Windows)',
        () {
          if (Platform.isWindows) {
            final result = WindowsProcessService.getAllProcessPaths();
    ...
    group('performance', () {
      test(
        'hasProcessInDirectory completes quickly (Windows)',
        () async {
          if (Platform.isWindows) {
      ...
      test(
        'isProcessRunning completes quickly (Windows)',
        () async {
          if (Platform.isWindows) {
            final stopwatch = Stopwatch()..start();
