...
class _AppSidebarState extends State<AppSidebar> {
  ...
  String? _syncNavBadge() {
    final q = widget.syncQueueService;
    if (q == null || !q.hasQueue) return null;
    final active = q.activeQueue.length;
    if (q.isPaused) return 'PAUSE';
    if (q.isRunning) return '$active';
    final failed = q.queue
        .where((entry) => entry.state == SyncQueueEntryState.failed)
        .length;
