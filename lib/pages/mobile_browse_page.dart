...
class _MobileBrowsePageState extends State<MobileBrowsePage> {
  ...
  Future<void> _performSearch({bool resetPage = false}) async {
    ...
    try {
      ...
      _log('performSearch - response: ${response.total} total, '
  ...
  String _formatRequest(SearchRequest request) {
    ...
    if (request.offerType != null) parts.add('type=${request.offerType!.value}');
    ...
    if (request.price != null) {
      parts.add('price=${request.price!.min ?? 0}-${request.price!.max ?? '∞'}');
    }
  ...
  void _showFiltersBottomSheet() {
    _log('showFiltersBottomSheet - current filters: '
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _FiltersBottomSheet(
        ...
        onApply: (offerType, sortBy, sortDir, onSale, priceRange,
                  excludeBlockchain, pastGiveaways, isLowestPrice) {
          _log('filtersApplied - type=$offerType, sort=$sortBy/$sortDir, '
  ...
  int get _activeFilterCount {
    ...
    if (_sortBy != SearchSortBy.lastModifiedDate || _sortDir != SearchSortDir.desc) count++;
    return count;
  }
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          ...
          child: GameCard(
            ...
            originalPrice: offer.price?.totalPrice?.originalPrice,
  ...
  String? _getThumbnailUrl(Offer offer) {
    ...
    final thumbnail = offer.keyImages.where((img) => img.type == 'Thumbnail').firstOrNull;
    if (thumbnail != null) return thumbnail.url;

    final boxTall = offer.keyImages.where((img) => img.type == 'DieselGameBoxTall').firstOrNull;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  GestureDetector(
                    onTap: () => setState(() => _showMoreSorts = !_showMoreSorts),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Text(
                  ...
                  const SizedBox(height: 8),
                  GestureDetector(
                    onTap: () => setState(() => _showMoreTypes = !_showMoreTypes),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Text(
